...
const authenticateOidc = async (req: Request): Promise<Response> => {
  ...
  if (!header) {
    return Response.json({ message: "Missing Authorization" }, { status: 401, headers: corsHeaders });
  }
  if (!header.startsWith(HeaderPrefix)) {
    return Response.json({ message: "Unexpected format" }, { status: 401, headers: corsHeaders });
  }
  ...
  if (!unverified.iss) {
    return Response.json({ message: "Missing issuer" }, { status: 401, headers: corsHeaders });
  }
  if (!URL.canParse(unverified.iss)) {
    return Response.json({ message: "Invalid issuer" }, { status: 401, headers: corsHeaders });
  }
  const requestedIssuer = new URL(unverified.iss);
  const requestedIdp = await db.findIdp(requestedIssuer);
  if (requestedIdp === null) {
    return Response.json({ message: "Unknown Token issuer" }, { status: 401, headers: corsHeaders });
  }
  ...
  try {
    ...
    if (err instanceof JWTClaimValidationFailed) {
      return Response.json({ message: "Failed validation" }, { status: 401, headers: corsHeaders });
    }
    if (err instanceof JWTExpired) {
      return Response.json({ message: "Token Expired" }, { status: 401, headers: corsHeaders });
    }
    throw err;
  }
  ...
  if (!new RegExp(requestedIdp.subject).test(payload.sub)) {
    return Response.json({ message: "Unauthorised", sub: payload.sub }, { status: 403, headers: corsHeaders });
  }

  const user = await db.findUser(requestedIdp.issuer, payload.sub);
  if (user === null) {
    return Response.json({ message: "Unknown user" }, { status: 403, headers: corsHeaders });
  }

  // Return generated user token
  // TODO: Generate User Token, and return in request.
  return Response.json({ message: "Authenticated", data: payload, user }, { status: 200, headers: corsHeaders });
};
...
const getOidcConfig = async (): Promise<Response> => {
  ...
  return Response.json({ config } satisfies z.input<typeof oidcConfigResponse>, { headers: corsHeaders });
};
